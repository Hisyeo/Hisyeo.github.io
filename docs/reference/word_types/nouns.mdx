...
after prepositions (thematic objects). In this position, they represent the
...
simply "food" in apposition.

#### Genitive Phrases

A highly useful kind of thematic relation is the genitive phrase directly
following another noun phrase connected by the <HisyeoWord is='yë'/> particle.
...
There is a special coupling particle (<HisyeoWord is='kon'/>) that converts any
content word directly after it into a noun with abstract connotations that can
be translated into English as "matters of" or "acts of". Additionally, this form
is often used as the equivalent of simple gerunds where there's no additional
thematic context needed.
...
There are two more particles that can convert content words into nouns and they
are both agentive in nature. This means that the final coupled noun is
considered an agent of some action or state. For the first particle (<HisyeoWord is='öni'/>),
